...
@testable import WebRequestCombie
#if swift(>=5.0) && _runtime(_ObjC)
...
class WebRequestCombieTests: XCTestCase {
    ...
    var testURLBase: URL { return WebRequestCombieTests.testURLBase }
    var testURLSearch: URL { return WebRequestCombieTests.testURLSearch }
    
    static var uploadedData: [String: Data] = [:]
    var uploadedData: [String: Data] { return WebRequestCombieTests.uploadedData }
    ...
    override class func setUp() {
        ...
        repeat {
            do {
                ...
                WebRequestCombieTests.server = LittleWebServer(listener)
                retry = false
        ...
        WebRequestCombieTests.server?.defaultHost["/search"] = {
        ...
        WebRequestCombieTests.server?.defaultHost["/events"] = { (request: LittleWebServer.HTTP.Request) -> LittleWebServer.HTTP.Response in
        ...
        WebRequestCombieTests.server?.defaultHost.get["/testfiles/"] = LittleWebServer.FSSharing.share(resource: URL(fileURLWithPath: webRequestTestFolder))

        WebRequestCombieTests.server?.defaultHost.post["/upload"] = { (request: LittleWebServer.HTTP.Request,
        ...
        do {
            try WebRequestCombieTests.server?.start()
    ...
    override class func tearDown() {
        ...
        WebRequestCombieTests.server?.stop()
        super.tearDown()
    }
